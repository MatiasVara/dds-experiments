# create guest
# NOTE: in this guest you should change the host name to fedora30-guest-2
# $ hostnamectl set-hostname fedora30-guest-2
VM_IMAGE_2=./fedora30-demo-2.qcow2

sudo virsh destroy fedora30-guest-2
sudo virsh undefine fedora30-guest-2

sudo virt-install -d --memballoon model=none --name fedora30-guest-2 --import --ram 2048 --vcpus 2 --cpu host \
        --os-type linux\
        --graphics none\
        --disk bus=virtio,path=../setup-1/fedora30-demo-2.qcow2 --os-variant fedora29\
	--cputune vcpupin0.vcpu=0,vcpupin0.cpuset=2-3,vcpupin1.vcpu=1,vcpupin1.cpuset=2-3\
	--console pty,target_type=serial \
        --network network=default,model=virtio --noautoconsole
